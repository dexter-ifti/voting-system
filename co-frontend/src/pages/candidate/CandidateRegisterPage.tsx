...
export const CandidateRegisterPage = () => {
  ...
  return (
    <div className="max-w-md mx-auto py-16 px-6">
      <h1 className="text-2xl font-semibold mb-6">Candidate Registration</h1>
      <form onSubmit={submit} className="space-y-4">
        ...
        <div>
          <label className="block text-xs mb-1">Party</label>
          <input value={form.party} onChange={e=>setForm(f=>({...f,party:e.target.value}))} required className="w-full" />
        </div>
        <div>
          <label className="block text-xs mb-1">Manifesto</label>
          <textarea value={form.manifesto} onChange={e=>setForm(f=>({...f,manifesto:e.target.value}))} required className="w-full h-28" />
        </div>
        <div className="grid grid-cols-2 gap-4">
          <div>
            <label className="block text-xs mb-1">Age</label>
            <input type="number" value={form.age} onChange={e=>setForm(f=>({...f,age:parseInt(e.target.value)||30}))} required className="w-full" />
          </div>
          <div>
        ...
        <div>
          ...
          <input value={form.phone} onChange={e=>setForm(f=>({...f,phone:e.target.value}))} required className="w-full" />
        </div>
      ...
      <p className="text-xs text-slate-400 mt-4">Have an account? <Link to="/candidate/login" className="text-primary">Login</Link></p>
    </div>
  );
};
