...
interface AadharOTPVerificationProps {
  aadharNumber: string;
  email: string;
  otpKey: string;
  onVerificationSuccess: (data: { aadharNumber: string; email: string }) => void;
  onCancel: () => void;
  onResendOTP: () => void;
  loading?: boolean;
  resending?: boolean;
...
export const AadharOTPVerification: React.FC<AadharOTPVerificationProps> = ({
  aadharNumber,
  email,
  otpKey,
    ...
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4 z-50">
      <div className="w-full max-w-md bg-slate-800/90 backdrop-blur-xl border border-slate-700/50 rounded-2xl p-8 shadow-2xl">
        ...
        <div className="text-center mb-8">
          ...
          <h2 className="text-2xl font-bold text-white mb-2">Verify Your Aadhar</h2>
        ...
        <div className="bg-slate-900/50 border border-slate-700/50 rounded-xl p-4 mb-6">
          <div className="flex items-center justify-between">
            <span className="text-slate-400 text-sm">Aadhar Number:</span>
            <span className="text-white font-mono">
              {aadharNumber.replace(/(\d{4})(?=\d)/g, '$1 ')}
            </span>
          </div>
        </div>
